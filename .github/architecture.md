...
| **Authentication** | Token via `X-Ingest-Token` header; optional `Authorization: Bearer` header |
...
2. The service authenticates the request (token + optional bearer).
...
| `AUTH_SECRET` | no | — | Optional secondary auth via `Authorization` header |
...
- Tokens are passed via headers, never query parameters
